...
import { useMerchantBusinessActivities } from '../../Composables/useMerchantBusinessActivities.js'
...
const canManageActivities = computed(() => page.props.canManageActivities === true)
...
watch([searchQuery, sortColumn, sortDirection], () => {
    router.get(route('merchant.activities.index'), {
...
const { deleteForm, detachCategory } = useMerchantBusinessActivities()
        ...
        <div class="max-w-7xl mx-auto">
            <div class="mb-6 md:mb-8">
                <button type="button" class="text-body text-blue-600 mb-2 cursor-pointer" @click="router.visit(route('merchant.home'))">
                    ← {{ t('merchantActivities.backToHome') }}
                </button>
                <h1 class="text-page-title text-gray-900 dark:text-gray-100">{{ t('merchantActivities.pageTitle') }}</h1>
                <p class="mt-2 text-muted muted-color">{{ t('merchantActivities.pageSubtitle', { name: merchant.name }) }}</p>
            </div>

            <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-4 md:mb-6 p-3 md:p-4">
                <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                    <div class="w-full sm:w-96">
                        <input v-model="searchQuery" type="text" :placeholder="t('merchantActivities.searchPlaceholder')"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden relative">
                ...
                <Pagination
                    ...
                    routeName="merchant.activities.index"
