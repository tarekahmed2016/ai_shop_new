...
import MerchantMembershipsTable from '../../Components/Features/Merchants/MerchantMembershipsTable.vue'
import MerchantMembershipFormModal from '../../Components/Features/Merchants/MerchantMembershipFormModal.vue'
...
import { useMerchantMemberships } from '../../Composables/useMerchantMemberships.js'
...
watch([searchQuery, sortColumn, sortDirection], () => {
    router.get(route('merchants.memberships.index', merchant.value.public_id), {
...
const { deleteForm, deleteMembership } = useMerchantMemberships(page.props.merchant.public_id)
        ...
        <div class="max-w-7xl mx-auto">
            <div class="mb-6 md:mb-8">
                <button type="button" class="text-body text-blue-600 mb-2 cursor-pointer" @click="router.visit(route('merchants.index'))">
                    ← {{ t('merchantMemberships.backToMerchants') }}
                </button>
                <h1 class="text-page-title text-gray-900 dark:text-gray-100">{{ t('merchantMemberships.pageTitle') }}</h1>
                <p class="mt-2 text-muted muted-color">{{ t('merchantMemberships.pageSubtitle', { name: merchant.name }) }}</p>
            </div>

            <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-4 md:mb-6 p-3 md:p-4">
                <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                    <div class="w-full sm:w-96">
                        <input v-model="searchQuery" type="text" :placeholder="t('merchantMemberships.searchPlaceholder')"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden relative">
                ...
                <MerchantMembershipsTable :memberships="memberships" :sortColumn="sortColumn" :sortDirection="sortDirection"
                    @edit="formModal.open" @delete="deleteModal.open" @sort="handleSort" />
                <Pagination :paginationData="paginationData" routeName="merchants.memberships.index" :routeParams="{ merchant: merchant.public_id }" @paginating="handlePaginating" />
            </div>
        </div>

        <MerchantMembershipFormModal
