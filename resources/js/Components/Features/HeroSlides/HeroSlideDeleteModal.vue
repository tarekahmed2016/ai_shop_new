<template>
  <DashboardModalShell
    :isOpen="isOpen"
    title-id="hero-slide-delete-modal-title"
    @close="handleClose"
  >
      <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 id="hero-slide-delete-modal-title" class="text-card-title text-gray-900 dark:text-gray-100">
              {{ t('heroSlides.deleteModal.title') }}
            </h2>
          </div>
        </div>
      </div>
      ...
      <div class="px-6 py-4">
        <p class="text-body text-gray-700 dark:text-gray-300">
          {{ t('heroSlides.deleteModal.confirmMessage', { name: displayTitle }) }}
        </p>
      </div>
      ...
      <div class="flex justify-end gap-3 px-6 py-4 bg-gray-50 dark:bg-gray-900 rounded-b-lg">
        ...
        >
          {{ t('heroSlides.deleteModal.cancel') }}
        </button>
        <button
          type="button"
          @click="handleConfirm"
        ...
        >
          {{ loading ? t('heroSlides.deleteModal.deleting') : t('heroSlides.deleteModal.deleteButton') }}
        </button>
      </div>
  </DashboardModalShell>
</template>
...
const displayTitle = computed(() =>
  resolveBilingualField(props.heroSlide, 'title', locale.value) || `#${props.heroSlide?.id || ''}`
)
