...
watch(() => props.isOpen, (isOpen) => {
  ...
  form.reset()
  form.customer_public_ids = [...props.selectedPublicIds]
  form.amount = 3
...
const submit = () => {
  form.customer_public_ids = [...props.selectedPublicIds]
  form.post(route('customers.extra-requests.bulk'), {
    preserveScroll: true,
    onSuccess: () => {
      emit('success')
      emit('close')
...
<template>
  <DashboardModalShell
    :isOpen="isOpen"
    title-id="customer-extra-request-bulk-modal-title"
    max-width-class="max-w-lg"
    @close="handleClose"
  >
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
      <h2 id="customer-extra-request-bulk-modal-title" class="text-card-title text-gray-900 dark:text-gray-100">
        {{ t('customers.extraRequests.bulkTitle') }}
      </h2>
    ...
    <form class="px-6 py-4 space-y-4" @submit.prevent="submit">
      <p class="text-muted muted-color">
        {{ t('customers.extraRequests.bulkSelected', { count: selectedPublicIds.length }) }}
      </p>
      <p v-if="form.errors.customer_public_ids" class="form-error">{{ form.errors.customer_public_ids }}</p>
      <div>
      ...
      <div>
        ...
        <p class="text-muted text-sm mt-1">{{ t('customers.extraRequests.paidAmountPerCustomerHint') }}</p>
      ...
      <div>
        <label class="form-label text-label">{{ t('customers.extraRequests.notesLabel') }}</label>
