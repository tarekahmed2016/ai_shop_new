<template>
  <DashboardModalShell
    :isOpen="isOpen"
    title-id="contact-message-delete-modal-title"
    @close="handleClose"
  >
      <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 id="contact-message-delete-modal-title" class="text-card-title text-gray-900 dark:text-gray-100">
              {{ t('contactMessages.deleteModal.title') }}
            </h2>
          </div>
        </div>
      </div>
      ...
      <div class="px-6 py-4">
        <p class="text-body text-gray-700 dark:text-gray-300">
          {{ t('contactMessages.deleteModal.confirmMessage', { name: contactMessage?.name || '' }) }}
        </p>
      </div>
      ...
      <div class="flex justify-end gap-3 px-6 py-4 bg-gray-50 dark:bg-gray-900 rounded-b-lg">
        ...
        >
          {{ t('contactMessages.deleteModal.cancel') }}
        </button>
        <button
          type="button"
          @click="handleConfirm"
        ...
        >
          {{ loading ? t('contactMessages.deleteModal.deleting') : t('contactMessages.deleteModal.deleteButton') }}
        </button>
      </div>
  </DashboardModalShell>
</template>
