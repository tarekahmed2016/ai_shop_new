        ...
        <table class="w-full border-collapse">
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="membership in memberships" :key="membership.id" class="table-row">
                    ...
                    <td class="table-cell table-cell-secondary text-body">{{ membership.user?.phone || '—' }}</td>
                    ...
                    <td class="table-cell table-cell-secondary text-body">{{ formatDate(membership.created_at) }}</td>
                    <td class="table-cell table-cell-actions">
                        <template v-if="membership.can_edit || membership.can_remove">
                            <button
                            ...
                            >
                                {{ t('merchantTeam.table.edit') }}
                            </button>
                            <button
                                v-if="membership.can_remove"
                                type="button"
                                @click="$emit('delete', membership)"
                                class="btn btn-danger"
                            >
                                {{ t('merchantTeam.table.remove') }}
                            </button>
                        </template>
                        <span v-else class="text-muted text-sm">{{ t('merchantTeam.table.viewOnly') }}</span>
                    </td>
                </tr>
            </tbody>
        </table>

        <EmptyState v-if="memberships.length === 0" :title="t('merchantTeam.noMembersFound')" />
    </div>
</template>
...
const columns = computed(() => [
    { key: 'name', label: t('merchantTeam.table.name'), sortable: false },
    { key: 'email', label: t('merchantTeam.table.email'), sortable: false },
    { key: 'phone', label: t('merchantTeam.table.phone'), sortable: false },
    { key: 'role', label: t('merchantTeam.table.role'), sortable: false },
    { key: 'status', label: t('merchantTeam.table.status'), sortable: true },
    { key: 'created_at', label: t('merchantTeam.table.joined'), sortable: true },
])
