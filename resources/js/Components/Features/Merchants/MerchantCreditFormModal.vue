...
const props = defineProps({
  ...
  merchantPublicId: { type: String, default: '' },
})
...
const submit = () => {
  ...
  if (props.mode === 'deduct') {
    form.transform((data) => ({
      ...
      notes: data.notes,
    })).post(route('merchants.credits.deduct', props.merchantPublicId), options)
    return
  }

  form.post(route('merchants.credits.store', props.merchantPublicId), options)
}
...
<template>
  <DashboardModalShell
    :isOpen="isOpen"
    title-id="merchant-credit-form-modal-title"
    max-width-class="max-w-lg"
    @close="handleClose"
  >
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
      <h2 id="merchant-credit-form-modal-title" class="text-card-title text-gray-900 dark:text-gray-100">
        {{ mode === 'deduct' ? t('merchants.credits.deductTitle') : t('merchants.credits.addTitle') }}
      </h2>
    ...
    <form class="px-6 py-4 space-y-4" @submit.prevent="submit">
      ...
      <div v-if="mode !== 'deduct'">
        <label class="form-label text-label">{{ t('merchants.credits.paidAmountLabel') }}</label>
        <input v-model="form.paid_amount" type="number" min="0" step="0.001" inputmode="decimal" class="form-input text-body" />
        <p class="text-muted text-sm mt-1">{{ t('merchants.credits.paidAmountHint') }}</p>
      ...
      <div>
        <label class="form-label text-label">
          {{ t('merchants.credits.notesLabel') }}
